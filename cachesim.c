...
int main(int argc, char *argv[])
{
    ...
    if(strncmp(argv[1], "direct", 6)==0){
          /* Simulating direct-mapped cache */
            ...
            fclose(fp);
            print_direct_cache_det(num_sets,way_size,&d_cache,argv[1]);
           ...
           /* Simulating fully associative cache */
            ...
            fclose(fp);
            print_kway_cache_det(num_sets,way_size,&k_cache,argv[1]);
...
void kway_associative_cache_access(kway_mapped_cache *cache, uint64_t address, int num_sets,int way_size)
{
    ...
    int random =0;
    //  printf("Memory address: %llu, Block address: %llu, set_num: %llu, Tag: %llu \n", address, block_addr, set_num, tag);
    ...
    for(int n=0; n<way_size;n++)
    {
        ...
        {
            printf("COMPULSORY MISSES : Memory address: %llu, Block address: %llu, set_num: %llu, Tag: %llu ", address, block_addr, set_num, tag);
    ...
    { /* Cache hit */
        printf("HITS : Memory address: %llu, Block address: %llu, set_num: %llu, Tag: %llu ", address, block_addr, set_num, tag);
         cache->hits += 1;
         break;
    }
#ifdef DBG
    ...
    {
        printf("REPLACEMENT : Memory address: %llu, Block address: %llu, set_num: %llu, Tag: %llu ", address, block_addr, set_num, tag);
